...
//  Set
//
//  Created by Anna Movsheva on 2/23/19.
...
class Set
{
    ...
    init(isPhonePlaying: Bool) {
        ...
        for n in 0...2 {
            for p in 0...2 {
                for c in 0...2 {
                    for s in 0...2 {
                        deck.append(Card(number: n, pattern: p, color: c,
                                         shape: s))
                    }
                }
            }
    ...
    */
    public func selection(of card: Card) -> Bool {
    ...
    */
    public func isSet(cards: [Card]) -> Bool {
        if cards.count != 3 {
            return false
        }
        var eq: (Card.Attributes, Card.Attributes) -> Int
        eq = { ($0 == $1).int }
        var makesSet: (Card.Attributes, Card.Attributes, Card.Attributes) -> Bool
        makesSet = { (eq($0, $1) + eq($1, $2) + eq($0, $2)) % 3 == 0 }
        let card0 = cards[0]
        let card1 = cards[1]
        let card2 = cards[2]
        for index in 0...3 {
